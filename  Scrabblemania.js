...
function wordscore(word) {
  ...
  for(let i = 0; i < wordIntoLetters.length; i++) {
    sumOfLetters += letterPointsSystem[wordIntoLetters[i]];
 }
  
  let result =  sumOfLetters * wordIntoLetters.length;
  
  if (wordIntoLetters.length === 7) { return result += 50} 
  return result;
}
// PSEUDOCODE:
//  user has to create a word from the letters in their hand, 
// create a function which returns the correct score for the word played by the user?
 
// The score of the word is the sum of the points of all the letters;
// multiplied by the number of letters in the word.

// If they use all their letters then they get a fifty point bonus. (if check)
